...
## 🎯 타겟층
유통회사 등에서 회원 및 재고 관리를 담당하는 공장주, 관리자, 사원 등 실무자  
...
- **[Database]**
  ...
  - **SQL 반영 작업**
    - 추가 설계한 테이블에 대한 DDL SQL문 작성 및 자신의 개발 작업에 따른 변경으로 일부 테이블 생성 SQL 문을 수정하고 프로젝트에 반영
...
- **유효성 검사**  
  ...
  - MVVM 패턴에 맞춰 텍스트박스와 바인딩하여 입력값 변경 시 정규표현식으로 실시간 검사  
  - 주요 검사 내용:  
...
- **비밀번호 암호화**  
  - 평문 저장 금지  
  - `BCrypt.Net-Next` 라이브러리 사용, 단방향 해시(암호화) 후 DB 저장  
  - 로그인 시 입력 비밀번호와 저장된 해시값 비교하여 인증 수행
